...
CAND_OUTDIR="${ROOT_OUTDIR}/aging-sjf"
...
if [[ -n "$PROFILE" ]]; then
  case "$PROFILE" in
    ...
    default)
      ;;
    *)
      echo "Unknown PROFILE: $PROFILE (supported: default, low, pressure, split)."
      exit 1
      ;;
  esac
fi
...
run_group() {
  ...
  local cand_dir="${ROOT_OUTDIR}/${group_label}/aging-sjf"
  ...
  run_case "aging-sjf" "$cand_dir" "$request_rates" "${candidate_args_ref[@]}"

  python3 tools/bench_compare_runs.py \
    "$base_dir" \
    "$cand_dir" \
    --baseline-name "baseline" \
    --candidate-name "aging-sjf"
}
...
if [[ "$PRESSURE_SPLIT" == "1" ]]; then
  ...
  run_case "aging-sjf" "$CAND_OUTDIR" "$REQUEST_RATES" "${CANDIDATE_SCHED_ARGS[@]}"
