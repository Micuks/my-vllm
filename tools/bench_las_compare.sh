...
run_group() {
  ...
  local cand_dir="${ROOT_OUTDIR}/${group_label}/las"
  ...
  run_case "las" "$cand_dir" "$request_rates" "${candidate_args_ref[@]}"
...
if [[ "$TRI_SPLIT" == "1" ]]; then
  ...
  run_case "las" "$CAND_OUTDIR" "$REQUEST_RATES" "${CANDIDATE_SCHED_ARGS[@]}"
